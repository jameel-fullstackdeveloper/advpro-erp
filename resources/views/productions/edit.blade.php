...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
            <h4 class="mb-sm-0 card-title">Edit Prodcution</h4>
...
<div class="row">
    <div class="col-12">
     ...
     <div class="card">
        ...
        <h5 class="mb-3 mt-2 text-uppercase"><i class=" ri-pencil-line"></i> Batch Detail <span class="text-info fs-13"></h5>



            <div class="col-md-2">
                                <label for="production_date" class="form-label">Date</label>
                                <input type="date" id="production_date" name="production_date" class="form-control" value="{{ old('production_date', $production->production_date) }}" required />
                ...
                <select id="finished_product" name="finished_product" class="form-select" required>
                    ...
                    @foreach ($finishedProducts as $product)
                        <option value="{{ $product->id }}"
                                {{ old('finished_product', $production->product_id) == $product->id ? 'selected' : '' }}>
                            {{ $product->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-5 mb-3">
                ...
                <input type="text" id="comments" name="comments" class="form-control" placeholder=""
                    value="{{ old('comments', $production->comments) }}" />
                    @error('comments')
                        <div class="text-danger">{{ $message }}</div>
                     @enderror
            </div>
    ...
    <tbody>
        <tr>
            <td>
                <label for="lots" class="form-label fs-13">Batch Excecuted</label>
                <input type="number" id="lots" name="lots" class="form-control text-center" placeholder=""
                    value="{{ old('lots', $production->lots) }}" required />
                @error('lots')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </td>
            ...
            <td>
                <label for="perlots" class="form-label fs-13">Per Batch <small>(Bags)</small></label>
                <input type="number" id="perlots" name="perlots" class="form-control text-center" placeholder=""
                    value="{{ old('perlots', $production->perlots) }}" required />
                @error('perlots')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </td>
            ...
            <td>
                <label for="defaultbags_perlot" class="form-label fs-13">Expected Production <small>(Bags)</small></label>
                <input type="number" id="defaultbags_perlot" name="defaultbags_perlot" class="form-control text-center" placeholder=""
                    value="{{ old('defaultbags_perlot', $production->defaultbags_perlot) }}" readonly required />
            ...
            <td>
                <label for="short_perlot" class="form-label text-danger fs-13">Shortage <small>(Bags)</small></label>
                <input type="number" id="short_perlot" name="short_perlot" class="form-control text-center" placeholder=""
                    value="{{ old('short_perlot', $production->short_perlot) }}" />
                @error('short_perlot')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </td>
            ...
            <td>
                <label for="excess_perlot" class="form-label text-success fs-13">Excess <small>(Bags)</small></label>
                <input type="number" id="excess_perlot" name="excess_perlot" class="form-control text-center" placeholder=""
                    value="{{ old('excess_perlot', $production->excess_perlot) }}" />
                @error('excess_perlot')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </td>
            ...
            <td>
                <label for="actual_produced" class="form-label fs-13">Actual Production <small>(Bags)</small></label>
                <input type="number" id="actual_produced" name="actual_produced" class="form-control text-center" placeholder=""
                    value="{{ old('actual_produced', $production->actual_produced) }}" readonly required />
                ...
                @enderror
                    ...
                    @foreach ($purchaseItemGroups as $group)
                        ...
                        <div class="card">
                        <div class="card-body">
                                <h5 class="mb-3 mt-2 text-uppercase"><i class=" ri-pencil-line"></i>  {{ $group->name }} <span class="text-info fs-13">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            @foreach ($group->purchaseItems as $item)
                                                <td>
                                                    <label for="material_{{ $item->id }}" class="form-label d-block" style="font-size:11px;">{{ $item->name }}</label>
                                                    <input type="number" id="material_{{ $item->id }}" name="raw_materials[{{ $item->id }}][quantity_used]" class="form-control text-center" step="0.01"
                                                           value="{{ old('raw_materials.' . $item->id . '.quantity_used', optional($production->details->where('raw_material_id', $item->id)->first())->quantity_used ?? 0) }}">
...
<div class="col-md-12">
<button type="submit" class="btn btn-primary btn-submit mt-3">Update Production</button>
</div>
