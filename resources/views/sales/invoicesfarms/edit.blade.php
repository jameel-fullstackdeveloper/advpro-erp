...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 card-title">Edit Sale (Farm)</h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="javascript: void(0);">Sales</a></li>
                    <li class="breadcrumb-item active"><a href="{{ url('sales/invoices') }}"> Sale (Farm) </a> > Edit</li>
                </ol>
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col">
        ...
        <form action="{{ route('invoicesfarms.update', $invoice->id) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="invoice-section">
                    ...
                    <select name="farm_account" class="form-select" id="farm_account" required>
                        <option value="">---Select Farm Account ---</option>
                            @foreach($farmaccounts as $farmaccount)
                               <option value="{{ $farmaccount->id }}" {{ old('farm_account', $invoice->farm_account) == $farmaccount->id ? 'selected' : '' }}>{{ $farmaccount->name }}</option>
                            @endforeach
                        </select>
        ...
        <div class="row">
            ...
            <div class="col-2">
                ...
                <input type="text" name="vehicle_no" class="form-control table-input" value="{{ old('vehicle_no', $invoice->vehicle_no) }}">
                @error('vehicle_no') <span class="text-danger">{{ $message }}</span> @enderror
            </div>

            <div class="col-2">
            <label for="vehicle_fare">Freight</label>
            <input type="number" name="vehicle_fare" class="form-control table-input"  oninput="toggleVehicleFareAdjRequired()" value="{{ old('vehicle_fare', $invoice->vehicle_fare) }}">
