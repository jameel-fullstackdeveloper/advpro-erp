...
class InvoiceFarmController extends Controller
{
 ...
 /**
    ...
    public function index(Request $request)
    {
        ...
        return view('sales.invoicesfarms.create', compact('products', 'customers', 'brokers', 'invoiceNumber','segments','farmaccounts'));
    }
    ...
    public function store(Request $request) {
        ...
        $allowedYear = $currentYear - 2;
            ...
            $validated = $request->validate([
                ...
                'invoice_number' => 'required|unique:sales_invoice_farms,invoice_number',
            ...
            try {
                ...
                $invoice = SalesInvoiceFarm::create($invoiceData);

                // Save invoice items
                foreach ($request->items as $item) {
                    SalesInvoiceFarmItem::create([
                        //'sales_invoice_id' => $invoice->id,
                        'sales_invoice_farm_id' => $invoice->id,
                ...
                $voucher = Voucher::create([
                    'voucher_type' => 'sales-invoice-farm',
                ...
                $salesAccount = $request->farm_account; // Sales account ID
            ...
            session()->flash('message', 'Sales Invoice(Farm) saved successfully!');
            return redirect('farms/invoicesfarms');
    ...
    private function generateInvoiceNumber()
    {
        ...
        $invoiceCount = SalesInvoiceFarm::where('invoice_date', '>=', $fiscalYearStart)
        ...
        do {
            ...
            $invoiceNumber =  'INV-FRM'  . $currentYear . '-' . $formattedNumber;

            // Check if the invoice number already exists in the database (including the fiscal year logic)
            $invoiceExists = SalesInvoiceFarm::where('invoice_number', $invoiceNumber)->exists();
    ...
    {
        $companyId = session('company_id'); // Retrieve the company_id from the session
        $invoice = SalesInvoiceFarm::with(['items'])->where('company_id', $companyId)->findOrFail($id);
     ...
     //   dd($salesOrders);
        ...
        return view('sales.invoicesfarms.edit', compact('invoice', 'products', 'customers', 'brokers','segments','farmaccounts'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $invoice = SalesInvoiceFarm::findOrFail($id);
