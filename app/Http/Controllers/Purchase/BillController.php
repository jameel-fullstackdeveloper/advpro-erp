...
class BillController extends Controller
{
 ...
 /**
    ...
    public function create(Request $request) {
        ...
        return view('purchase.bills.create',compact('vendors','billNumber' , 'brokers','segments'));
    ...
    private function generateBillNumber()
    {
        ...
        do {
            ...
            $billNumber =  'PB-FM'  . $currentYear . '-' . $formattedNumber;
    ...
    {
        // Retrieve all purchase orders associated with the selected vendor
        $orders = PurchaseOrder::where('vendor_id', $vendorId)
                    ->where('status', '!=', 'completed')
                    ->get(['id', 'order_number', 'order_date', 'status']); // Adjust fields as needed
        ...
        // Return an error response if no orders are found for the vendor
        return response()->json(['success' => false, 'message' => 'No orders found for this vendor'], 404);
    }

    public function getOrderItems($orderId)
    {
        // Retrieve the purchase order based on the provided order ID
        ...
        // Retrieve the items for the selected purchase order
        $items = PurchaseOrderItem::where('purchase_order_id', $orderId)->get();

        // Map the items to the required format
        $orderItems = $items->map(function ($item) {
            return [
                'product_id' => $item->product_id,
                'product_name' => $item->product->name, // Access 'item_name' field
                'quantity' => $item->quantity,
                'price' => $item->price,
            ];
        });

        // Return the order items in the response
        return response()->json([
            'success' => true,
            'order' => [
                'delivery_mode' => $purchaseOrder->delivery_mode,
                'bill_due_days' => $purchaseOrder->credit_days,
                //'broker_id' => $purchaseOrder->broker_id,
            ],
            'items' => $orderItems,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'order_id' => 'required', // Ensure order exists
    ...
    public function edit($id)
    {
        ...
        $purchaseBill = PurchaseBill::with('items.product')->findOrFail($id);
        $vendors = ChartOfAccount::where('is_customer_vendor', 'vendor')
        ...
        return view('purchase.bills.edit', compact('purchaseBill', 'vendors', 'brokers', 'purchaseOrders'));
    }
