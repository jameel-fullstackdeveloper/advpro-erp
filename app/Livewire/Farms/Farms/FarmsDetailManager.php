...
class FarmsDetailManager extends Component
{
    ...
    public function store()
    {
        ...
        session()->flash('message', $this->customerId ? 'Farm updated successfully.' : 'Farm added successfully.');
        $this->dispatch('hideModal_customer');
    }


    public function confirmDeletion($farmId)
    {
        // This method can be used to confirm and trigger the deletion
        $this->dispatch('swal:confirm-deletion-farm', farmId: $farmId);
    }
    ...
    public function render()
    {
        ...
        $customers = CustomerDetail::whereHas('coaTitle', function($query) use ($companyId) {
                $query->where('is_customer_vendor', 'farm') // Only fetch records where it's marked as a customer
                    ->where('company_id', $companyId); // Ensure it's for the correct company
            })
            ->with(['coaTitle' => function($query) use ($companyId) {
                $query->where('is_customer_vendor', 'farm')
                    ->where('company_id', $companyId); // Ensure it's for the correct company
            }])
            ->where(function($query) use ($searchTerm, $companyId) {
                $query->whereHas('coaTitle', function($query) use ($searchTerm, $companyId) {
                    $query->where('name', 'like', $searchTerm)
                        ->where('company_id', $companyId); // Search within ChartOfAccount's name
                })
        ...
        return view('livewire.farms.farms.farms-detail-manager', ['customers' => $customers]);
    }
