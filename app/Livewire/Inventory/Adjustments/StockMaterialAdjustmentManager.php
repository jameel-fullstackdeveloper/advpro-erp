...
class StockMaterialAdjustmentManager extends Component
{
    use WithPagination;
    ...
    {
        $stockMaterialAdjustments = StockMaterialAdjustment::query()
        ->with('purchaseItem') // Eager load the related purchase item
        ->where(function ($query) {
            if ($this->searchTerm) {
                $query->whereHas('purchaseItem', function($q) {
        ...
        return view('livewire.inventory.adjustments.show', [
    ...
    public function store()
    {
        ...
        if ($this->isEditMode) {
            $stockMaterialAdjustment = StockMaterialAdjustment::find($this->stockMaterialAdjustmentId);
            ...
            StockMaterialAdjustment::create([
                'adj_date' => $this->adj_date,
                'material_id' => $this->material_id,
                'shortage' => $this->shortage,
                'exccess' => $this->exccess,
    ...
    {
        $stockMaterialAdjustment = StockMaterialAdjustment::find($id);
