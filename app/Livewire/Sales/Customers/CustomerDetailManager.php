...
class CustomerDetailManager extends Component
{
    ...
    public function store()
    {
        ...
        $chartOfAccount = ChartOfAccount::updateOrCreate(
            ...
            [
                ...
                'drcr' => $this->drcr,
                'is_farm' => $this->is_farm, // Add this line to store is_farm
    ...
    public function render()
    {
        ...
        $customers = CustomerDetail::whereHas('coaTitle', function($query) use ($companyId) {
                ...
                ->where('is_farm', 0); // Ensure it's for the correct company
            })
            ...
            ->where(function($query) use ($searchTerm, $companyId) {
                $query->whereHas('coaTitle', function($query) use ($searchTerm, $companyId) {
                    ...
                    ->where('is_farm', 0);  // Search within ChartOfAccount's name
                })
       ...
       // dd($customers);
        ...
        return view('livewire.sales.customers.customer-detail-manager', ['customers' => $customers]);
    }
    ...
    public function deleteCustomer($id)
    {
        DB::transaction(function () use ($id) {
            ...
            if ($voucherExists) {
                session()->flash('error', 'Customer cannot be deleted because they have associated records.');
                return;
            }
