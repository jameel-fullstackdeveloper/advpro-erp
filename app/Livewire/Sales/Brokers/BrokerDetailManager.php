...
namespace App\Livewire\Sales\Brokers;
...
class BrokerDetailManager extends Component
{
    ...
    public $customerId;
    public $account_name;
    public $group_id=10; // General Creditor
    public $balance=0;
    public $cnic;
    public $strn;
    public $ntn;
    ...
    public function mount()
    {
        ...
        abort_if(!auth()->user()->can('sales brokers view'), 403);
    }
    ...
    public function store()
    {
        ...
        $chartOfAccount = ChartOfAccount::updateOrCreate(
            ...
            [
                ...
                'is_customer_vendor' => 'sales_broker',
    ...
    public function render()
    {
        ...
        $customers = CustomerDetail::whereHas('coaTitle', function($query) use ($companyId) {
                $query->where('is_customer_vendor', 'sales_broker')
                ->where('company_id', $companyId); // Only fetch records where it's marked as a customer
            })
            ->with(['coaTitle' => function($query) use ($companyId) {
                $query->where('is_customer_vendor', 'sales_broker')
                ->where('company_id', $companyId); // Ensure it only loads customer accounts
            }])
        ...
        return view('livewire.sales.brokers.broker-detail-manager', ['customers' => $customers]);
    }
