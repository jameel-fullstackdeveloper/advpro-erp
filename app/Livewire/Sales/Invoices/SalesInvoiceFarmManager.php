...
class SalesInvoiceFarmManager extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        $invoices = SalesInvoiceFarm::with(['items'])
           ...
           ->orderBy('sales_invoice_farms.id', 'desc') // Order by the most recent order_date
       ...
       $products = Items::where(function ($query) {
        ...
        return view('livewire.sales.invoices.sales-invoice-farm-manager', compact('invoices', 'products', 'customers','filter_groups' ,'farms'));
    }
    ...
    public function store()
    {
        try {
            ...
            if ($this->isEditMode) {
                ...
                $invoice = SalesInvoiceFarm::find($this->invoiceId);
            ...
            if ($existingVoucher) {
                ...
                // Create new invoice
                $invoice = SalesInvoiceFarm::create($data);

            }

            // Save invoice items
            foreach ($this->items as $item) {
                SalesInvoiceFarmItem::updateOrCreate(
                    ['id' => $item['id'] ?? null],
                    [
    ...
    public function edit($id)
    {
        ...
        $invoice = SalesInvoiceFarm::with(['items'])->findOrFail($id);
    ...
    private function generateInvoiceNumber()
    {
        ...
        // Count the number of vouchers in the current financial year
        $voucherCount = SalesInvoiceFarm::where('invoice_date', '>=', $financialYearStart)->count();
        $nextVoucherNumber = $voucherCount + 1;
        ...
        do {
            ...
            // Check if the voucher number already exists
            $voucherExists = SalesInvoiceFarm::where('invoice_number', $voucherNumber)->exists();
    ...
    public function delete($id)
    {
        ...
        SalesInvoiceFarmItem::where('sales_invoice_farm_id', $id)->delete();
