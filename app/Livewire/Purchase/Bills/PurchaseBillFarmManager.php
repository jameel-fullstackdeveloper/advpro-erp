...
class PurchaseBillFarmManager extends Component
{
    use WithPagination;
    ...
    public function store()
    {
        try {
            ...
            if ($this->isEditMode) {
                ...
                $purchaseBill = PurchaseFarmBill::findOrFail($this->billId);
                ...
                PurchaseFarmBillItem::where('purchase_bill_id', $purchaseBill->id)->delete();
    ...
    // Ensure filter_items is always an array
        ...
        $query = PurchaseFarmBill::with(['vendor', 'items.product']);
        ...
        // Apply farm filter (if selected)
        if ($this->selectedFarm) {
            $query->where('farm_account', $this->selectedFarm); // Assuming `farm_account` is the relationship to the farm
        }



        // Fetch the filtered purchase bills with pagination
        $purchaseBills = $query->orderBy('purchase_farm_bills.id', 'desc')
        ...
        return view('livewire.purchase.billsfarms.purchase-bill-farm-manager', [
    ...
    {
        $purchaseBill = PurchaseFarmBill::with('items')->findOrFail($id);
    ...
    public function deletePurchaseBill($id)
    {
        DB::transaction(function () use ($id) {
            ...
            PurchaseFarmBillItem::where('purchase_farm_bill_id', $purchaseBill->id)->delete();
